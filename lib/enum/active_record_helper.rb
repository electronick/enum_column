  ...
  module ActionView
    module Helpers
      ...
      module FormOptionsHelper
        #def select
        def enum_select(object, method, select_values={}, options={}, html_options={})
          InstanceTag.new(object, method, self, options.delete(:object)).to_enum_select_tag(options, select_values, html_options)
        end
      end

      class InstanceTag
        def to_enum_select_tag(options, select_values={}, html_options={})
          ...
          if select_values.empty?
            to_select_tag(options, html_options)
          else
            select_options = []
            column.limit.each do |limit|
              select_options << [select_values[limit] || limit, limit]
            end
            to_select_tag(select_options, options, html_options)
          end
        end
      ...
      class FormBuilder
        def enum_select(method, select_values={}, options={}, html_options={})
          @template.enum_select(@object_name, method, select_values, objectify_options(options), @default_options.merge(html_options))
        end
      end
    end
  end
